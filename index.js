...
// ─── International English: Listening & Speaking generation ──────────────────
app.post('/international-english/listening', authenticate, aiLimiter, async (req, res) => {
  try {
    const { level } = req.body;
    const lvl = (level || 'B1').toUpperCase();
    const prompt = `Create a CEFR ${lvl} English LISTENING practice section (independent practice material).
Write a short natural dialogue or monologue (about 150-220 words) appropriate to ${lvl}, then 5 comprehension questions about it.
Return ONLY valid JSON, no markdown:
{
  "title": "Listening — ${lvl}",
  "speakers": [{"name":"Narrator","gender":"female","accent":"British"}],
  "script": [{"speaker":"Narrator","text":"..."}],
  "questions": [{"id":1,"question":"...","options":["A ...","B ...","C ...","D ..."],"answer":"A ...","explanation":"..."}]
    ...
    let text = r.content[0].text.replace(/```json|```/g,'').trim();
    res.json(JSON.parse(text));
  } catch(e) { console.error('IE listening error:',e.message); res.status(500).json({error:e.message}); }
});

app.post('/international-english/speaking', authenticate, aiLimiter, async (req, res) => {
  try {
    const { level } = req.body;
    const lvl = (level || 'B1').toUpperCase();
    const prompt = `Create a CEFR ${lvl} English SPEAKING practice set (independent practice material).
Provide 4 spoken-response questions appropriate to ${lvl}, increasing in difficulty, on everyday and slightly abstract topics.
Return ONLY valid JSON, no markdown:
{ "title": "Speaking — ${lvl}", "questions": ["question 1","question 2","question 3","question 4"] }`;
    const r = await anthropic.messages.create({ model:'claude-sonnet-4-6', max_tokens:800, messages:[{role:'user',content:prompt}] });
    res.json(JSON.parse(r.content[0].text.replace(/```json|```/g,'').trim()));
  } catch(e) { console.error('IE speaking error:',e.message); res.status(500).json({error:e.message}); }
});

app.post('/international-english/speaking/grade', authenticate, aiLimiter, async (req, res) => {
  try {
    const { level, transcript } = req.body;
    const lvl = (level || 'B1').toUpperCase();
    const prompt = `You are an expert English examiner. Grade this spoken response at CEFR ${lvl} expectations.
Transcript of what the student actually said: "${transcript}"
Return ONLY valid JSON, no markdown:
{"level":"${lvl}","score":0,"fluency":{"score":0,"feedback":"..."},"vocabulary":{"score":0,"feedback":"..."},"grammar":{"score":0,"feedback":"..."},"pronunciationNote":"based only on word choice/structure since this is a transcript","overallFeedback":"2-3 sentences","strengths":["..."],"improvements":["...","..."]}
Use score out of 10 for each category and overall.`;
    const r = await anthropic.messages.create({ model:'claude-sonnet-4-6', max_tokens:800, messages:[{role:'user',content:prompt}] });
    res.json(JSON.parse(r.content[0].text.replace(/```json|```/g,'').trim()));
  } catch(e) { console.error('IE speaking grade error:',e.message); res.status(500).json({error:e.message}); }
});

// ─── SPEECH-TO-TEXT (Whisper) — converts recorded audio to real text ─────────
// Used by speaking practice so recorded answers are actually transcribed and graded.
app.post('/transcribe', authenticate, aiLimiter, async (req, res) => {
  try {
    const { audio, mimeType } = req.body;
    if (!audio) return res.status(400).json({ error: 'No audio provided' });

    // audio is base64 (without data: prefix). Decode to a buffer.
    const buffer = Buffer.from(audio, 'base64');
    if (buffer.length < 1000) return res.json({ text: '' }); // too short / empty recording
    ...
    const ext = (mimeType && mimeType.includes('mp4')) ? 'mp4'
              ...
              : 'webm';
    const file = await OpenAI.toFile(buffer, 'speech.' + ext, { type: mimeType || 'audio/webm' });
    ...
    // Graceful: return empty text so the UI can fall back to typed input
